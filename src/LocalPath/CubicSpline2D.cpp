...
CubicSpline2D::CubicSpline2D(const vector<f1_msgs::msg::Waypoint> &global_path) {
    s.clear();
...
void CubicSpline2D::setCubicSpline2D(const vector<f1_msgs::msg::Waypoint> &global_path) {
...
vector<vector<double>> CubicSpline2D::remove_collinear_points(const vector<f1_msgs::msg::Waypoint> &global_path) {
    ...
    for (size_t i = 2; i < global_path.size() - 1; i++) {
        bool collinear =
            are_collinear(global_path[i - 2].x, global_path[i - 2].y, global_path[i - 1].x, global_path[i - 1].y, global_path[i].x, global_path[i].y);
        if (collinear) {
            continue;
        }
